...
use crate::parser::ast::{
    AdditiveOperator, Argument, ArgumentList, ArrayItem, AssignmentExpression, AssignmentOperator,
    BindingType, CallExpressionType, ComplexPropertyAccessExpressionData, Declaration, Expression,
    FunctionCallExpressionType, Instruction, JsError, LhsExpressionType,
    LhsRhsAssignmentExpressionType, LiteralType, LogicAndMathExpression, LogicalEqualityOperator,
    MemberExpression, MultiplicativeOperator, NewExpressionType, NumericType, PostfixOperator,
    PrimaryExpressionType, RelationalOperator, Script, ShiftOperator,
    SimplePropertyAccessExpressionData, Statement, UnaryOperator,
};
...
impl JsParser {
    ...
    pub fn parse_to_ast(script: &str) -> Result<Script, JsError> {
        let result = Self::parse(Rule::script, script);
        match result {
            Ok(pairs) => build_ast_from_statement_list(pairs),
            Err(rule) => {
                return Err(JsError {
...
fn get_unexpected_error(pair: &Pair<Rule>) -> JsError {
...
fn build_ast_from_statement_list(pairs: Pairs<Rule>) -> Result<Script, JsError> {
...
fn build_ast_from_declaration(pair: Pair<Rule>) -> Result<Declaration, JsError> {
    ...
    match inner_pair.as_rule() {
        ...
        _ => return Err(get_unexpected_error(&inner_pair)),
    }
}

fn build_ast_from_lexical_declaration(pair: Pair<Rule>) -> Result<Declaration, JsError> {
    unimplemented!()
}

fn build_ast_from_hoistable_declaration(pair: Pair<Rule>) -> Result<Declaration, JsError> {
    unimplemented!()
}

fn build_ast_from_statement(pair: Pair<Rule>) -> Result<Statement, JsError> {
    let inner_pair = pair.into_inner().next().unwrap();
    Ok(match inner_pair.as_rule() {
        ...
        Rule::expression_statement | Rule::expression_statement__yield => unimplemented!(),
...
fn build_ast_from_variable_statement(pair: Pair<Rule>) -> Result<Statement, JsError> {
    ...
    for var_pair in pair.into_inner() {
        ...
        {
            declarations.push(build_ast_from_lexical_binding_or_variable_declaration(
                var_pair,
            )?)
        } else if var_pair.as_rule() != Rule::smart_semicolon {
            // smart_semicolon at the end is expected but nothing except smart_semicolon.
            return Err(get_unexpected_error(&var_pair));
        }
    }
    Ok(VariableStatement(declarations))
}

fn build_ast_from_lexical_binding_or_variable_declaration(
    pair: Pair<Rule>,
) -> Result<BindingType, JsError> {
    ...
    Ok(
            ...
            || lexical_binding_inner.as_rule() == Rule::binding_pattern__yield
        {
            ...
            return Err(get_unexpected_error(&lexical_binding_inner));
        },
    )
}
        ...
        Rule::left_hand_side_expression | Rule::left_hand_side_expression__yield => {
            ...
            let operator = if next_pair.as_rule() == Rule::assignment_operator {
                ...
                match op_str {
                    ...
                    _ => return Err(get_unexpected_error(&next_pair)),
                }
            } else {
    ...
    loop {
        if let Some(inner_pair) = pair_iter.next() {
            ...
            operators.push(match inner_pair_1.as_str() {
                ...
                _ => return Err(get_unexpected_error(&inner_pair_1)),
            });
            ...
            || first_pair.as_rule() == Rule::postfix_expression__yield
        {
            ...
            LogicAndMathExpression::UnaryExpression {
                operator: match first_inner_pair.as_str() {
                    ...
                    _ => return Err(get_unexpected_error(&first_inner_pair)),
                },
    ...
    if let Some(op_pair) = pair_iter.next() {
        Ok(LogicAndMathExpression::PostfixExpression {
            ...
            operator: match op_pair.as_str() {
                ...
                _ => return Err(get_unexpected_error(&op_pair)),
            },
        })
    } else {
        Ok(lhs)
...
fn build_ast_from_call_expression(pair: Pair<Rule>) -> Result<CallExpressionType, JsError> {
    ...
    for pair in pair_iter {
        obj = match pair.as_rule() {
            ...
            _ => return Err(get_unexpected_error(&pair)),
        };
    }
    Ok(obj)
}
    ...
    Ok(
            ...
            || pair_1.as_rule() == Rule::new_member_expression__yield
        {
            ...
            let mut obj = match pair_1.as_rule() {
                ...
                _ => return Err(get_unexpected_error(&pair_1)),
            };
            for pair in pair_iter {
                obj = match pair.as_rule() {
                    Rule::expression__in_yield | Rule::expression__in => {
        ...
        Rule::decimal_literal => {
            ...
            for decimal_pair in inner_pair.into_inner() {
                num = match decimal_pair.as_rule() {
                    ...
                    _ => return Err(get_unexpected_error(&decimal_pair)),
                }
            }
            if !is_float {
